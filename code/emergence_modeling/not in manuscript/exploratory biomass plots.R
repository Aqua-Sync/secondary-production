...
# compare estimates before and after addition of brm_multiple() -----------

old_preds = readRDS("posteriors - Copy/hybas_predictions_kgdm_peryear.rds") %>% mutate(source = "old")
new_preds = readRDS("posteriors/hybas_predictions_kgdm_peryear.rds") %>% mutate(source = "new")

bind_rows(old_preds, new_preds) %>% 
  ungroup %>% 
  select(HYBAS_ID, median, source) %>% 
