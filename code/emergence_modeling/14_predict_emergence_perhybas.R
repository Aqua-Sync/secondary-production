...
for(i in seq_along(data_to_predict_list)) {
  ...
  post_pufa_hybas_summary[[i]] = data_to_predict_list[[i]] %>%
    ...
    mutate(author_year = "new") %>%
    add_epred_draws(final_mod, allow_new_levels = TRUE, re_formula = NULL, ndraws = 100) %>%
    mutate(mgdmm2 = .epred*mean_emergence/1000/1000) %>% 
    select(-.epred) %>%
    left_join(post_pufa %>% ungroup %>% select(.draw, mean_ngPUFA_mgDM))  %>% 
...
post_flux_kgdm_perm2year_hybas = bind_rows(post_summary) %>% 
  ...
  mutate(kgdmkm2y = value*mean_emergence) %>% # kg/km2 is the same as mg/m2
  select(-value) %>% 
