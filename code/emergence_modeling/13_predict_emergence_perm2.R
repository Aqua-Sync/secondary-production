...
post_mass_nutrients = data_to_predict %>% 
  distinct(precip_s, stream_temp_s, HYBAS_ID, region, sd_emergence_1) %>% 
  group_by(region) %>% 
  ...
  add_epred_draws(mod, 
                  ...
                  ndraws = 500) %>% 
  ungroup %>%
  mutate(mgDMm2y = .epred*mean_emergence) %>% 
...
post_mass_nutrients_biome = data_to_predict %>% 
  distinct(precip_s, stream_temp_s, HYBAS_ID, region, terr_biom, sd_emergence_1) %>% 
  group_by(terr_biom) %>% 
  slice_sample(prop = 0.01) %>% 
  add_epred_draws(mod, 
                  re_formula = NULL, 
...
post_pufa = tibble(pub_name = "new",
  ...
  mutate(mean_ngPUFA_mgDM = .epred*unique(pufa_data$max_y),
         pufa_units = "ng_mg_dm") 
