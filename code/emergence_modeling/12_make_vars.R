...
data_to_predict = hydrobasin_vars_rssa_short %>% 
  as_tibble() %>% 
  mutate(sd_emergence_1 = 1000, # dummy value for mi() part of fitted models
         precip_s = (pre_mm_syr - attributes(emergence_production_with_vars$precip_s)[[2]])/attributes(emergence_production_with_vars$precip_s)[[3]],
