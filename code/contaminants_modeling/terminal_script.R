...
simulate_flux_sum <- function(iter_num, df) {
  ...
  sum_global <- sum(result$sum_hybas_cont)
  result[, `:=`(
...
# relative_flux %>% 
#   # sample_n(10000) %>%
#   select(HYBAS_ID, starts_with("median_"), lat) %>% 
#   pivot_longer(cols = c(starts_with("median"))) %>% 
#   mutate(value = case_when(name == "median_prop_contaminants" ~ -1*value,
#                            TRUE ~ value)) %>% 
#   ggplot(aes(x = lat, y = value, color = name)) +
#   geom_bar(stat = "identity")
# 
# relative_flux %>% 
#   mutate(diff = median_prop_nutrients - median_prop_contaminants) %>% 
#   sample_n(100000) %>% 
#   ggplot(aes(x = diff, y = median)) + 
#   geom_point(shape = ".") +
#   geom_abline()
# 
# relative_flux %>% 
...
#   viridis::scale_fill_viridis(limits = c(-2e-9, 2e-9),
#     oob = scales::squish) +
