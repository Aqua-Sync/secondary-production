...
hybas_area = readRDS("data/HYBAS_surface_area_REDIST.rds") %>% # redistributed surface areas from Jakob. 
  filter(HYBAS_ID %in% hybas_filtered)

total_km2 = sum(hybas_area$area.redist) # TOTAL km2 of water in sampled hybas locations

pest_herb_fungicide_perkm2_temp = global_predictions_pest_herb_fungicide %>% 
  mutate(global_flux_mg_peryr = global_flux_MT_peryr*1e+09,
         global_flux_mg_peryr_perkm2 = global_flux_mg_peryr/total_km2) %>% 
  group_by(chemical) %>% 
  median_qi(global_flux_mg_peryr_perkm2) %>% 
  mutate(code = "7) calculate_global_insecticide_herbicide_fungicide_flux_perm2.R")

sum_pest_herb_fungicide_perkm2 = global_predictions_pest_herb_fungicide %>% 
  group_by(.draw) %>% 
  reframe(global_flux_MT_peryr = sum(global_flux_MT_peryr)) %>% 
  mutate(global_flux_mg_peryr = global_flux_MT_peryr*1e+09,
         global_flux_mg_peryr_perkm2 = global_flux_mg_peryr/total_km2) %>%
  median_qi(global_flux_mg_peryr_perkm2) %>% 
  mutate(code = "7) calculate_global_insecticide_herbicide_fungicide_flux_perm2.R",
         chemical = "Sum Pesticides")

pest_herb_fungicide_perkm2 = bind_rows(pest_herb_fungicide_perkm2_temp,
                                       sum_pest_herb_fungicide_perkm2)

write_csv(pest_herb_fungicide_perkm2, file = "tables/pest_herb_fungicide_perkm2.csv")
