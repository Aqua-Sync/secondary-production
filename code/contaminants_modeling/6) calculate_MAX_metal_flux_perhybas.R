...
# This code generates aquatic concentrations of contaminants. max.conc.year is the log10 mean ug/L, mean.det.year is detection. 
# 1) load custom function

source("code/custom_functions/get_hybas_max_contaminant_preds.r") # function to combine biomass and contaminant concentrations, then multiply to get posterior prediction of contaminant flux in each HYBAS
...
hybas_max_contaminant_predictions = lapply(filtered_mod_list, get_hybas_max_contaminant_preds) 

saveRDS(hybas_max_contaminant_predictions, file = "posteriors/hybas_max_contaminant_predictions.rds")

hybas_max_contaminant_predictions = bind_rows(hybas_max_contaminant_predictions) %>% 
...
saveRDS(hybas_max_contaminant_predictions, file = "posteriors/hybas_max_predictions_metals.rds")


# split and save by separate elements

hybas_max_contaminant_predictions = readRDS(file = "posteriors/hybas_max_predictions_metals.rds") 

split_data_metals = split(hybas_max_contaminant_predictions, hybas_max_contaminant_predictions$element)

# Save each split tibble as an .rds file
lapply(names(split_data_metals), function(element) {
  filename <- paste0("posteriors/hybas_max_predictions_mgperyear_", element, ".rds")
  saveRDS(split_data_metals[[element]], file = filename)
})
