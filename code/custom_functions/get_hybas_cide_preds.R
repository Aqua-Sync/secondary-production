get_hybas_cide_preds = function(model){
  ...
  water_predictions = modeled_water %>% 
    filter(chemical_category == model$data2$chemical_category) %>% 
    mutate(chemical = chemical_category,
 ...
 flux_predictions_all %>% 
   ...
   # left_join(cas_names %>% select(cas, chemical_category)) %>% 
   mutate(chemical_category = model$data2$chemical_category) %>% 
   mutate(
